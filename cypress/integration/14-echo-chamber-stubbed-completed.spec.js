...
describe('Signing in with a seeded database', () => {
  beforeEach(() => {
    ...
    // if you make an API request to /echo-chamber/api/1232332323
    // return post.json as its response
    ...
    // essentially firstPost =
    // {
    //   "id": 1,
    //   "content": "Ravioli are a form of Pop Tart.",
    //   "createdAt": "2021-12-17T10:47:08.198Z",
    //   "authorId": 1,
    //   "author": { "id": 1, "email": "first@example.com" }
    // }
  });

  it('should render the posts from the API', () => {
    // posts mocked via fixtures
    cy.fixture('posts').then(({ posts }) => {
      cy.get('@previews').should('have.length', posts.length);
    });
  });

  it.only('should navigate to the URL of the post that you clicked on', () => {
    cy.get('@previews').first().click();
    cy.wait('@postApi').then((interception) => {
      // interception will have something like:
      // { response: { body: }}
  ...
  describe('An individual post', () => {
    ...
    it('should send a PATCH request when you send your edit', () => {
      ...
      cy.wait('@patchRequest'); // just checking for patch is enough
      // shouldn't you check that it updates?
    });

    it('should send a DELETE request when click on the delete button', () => {
      cy.get('[data-test="post-detail-controls-delete-button"]').click();
      cy.wait('@deleteRequest'); // just checking for delete is enough?
      // shouldn't you check that it goes away?
    });
  });
});
