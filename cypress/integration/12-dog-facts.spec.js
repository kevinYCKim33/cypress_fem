...
describe('Dog Facts', () => {
  beforeEach(() => {
    ...
    // <p data-test="empty-state">Fetch some dog facts or something.</p>
    cy.intercept('/dog-facts/api?*').as('api'); // literally says it's not stubbed
  });
  ...
  it.only('should show the correct number of facts on the page', () => {
    ...
    cy.get('@fetchButton').click();
    cy.get('[data-test="dog-fact"]').should('have.length', 6); // where is the mock response??
  });
